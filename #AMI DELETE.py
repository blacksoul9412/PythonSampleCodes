...
images = ec2.images.filter(Owners=["xxxx9772xxxx"])# Specify your AWS account owner id in place of "XXXXX" at all the places in this script
...
def lambda_handler(event, context):
    ...
    if backupSuccess == True:
        ...
        for image in imagesList:
            #print image
            desc_image_snapshots = ec.describe_images(ImageIds=[image],Owners=['437097720063',])['Images'][0]['BlockDeviceMappings']
           # print (desc_image_snapshots)
            try:
                for desc_image_snapshot in desc_image_snapshots:
                    snapshot = ec.describe_snapshots(SnapshotIds=[desc_image_snapshot['Ebs']['SnapshotId'],], OwnerIds=['437097720063'])['Snapshots'][0]
